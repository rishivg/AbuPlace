...
def basic_variable(macro_pos, macro_size, macro_fixed, die_info):
    ...
    for i in range(num_macros):
        x_value = macro_pos[i][0]
        y_value = macro_pos[i][1]
        # Clamp initial values to variable bounds to avoid float precision errors
        x_lo, x_hi = x_set[i].lowBound, x_set[i].upBound
        y_lo, y_hi = y_set[i].lowBound, y_set[i].upBound
        x_value = max(x_lo, min(x_hi, x_value))
        y_value = max(y_lo, min(y_hi, y_value))
