> **Note.** This is the original challenge description from the
> [Partcl/HRT Macro Placement Challenge](https://github.com/partcleda/partcl-macro-place-challenge),
> preserved here for context on what this placer was built to solve, how it is
> scored, and what the rules allowed. It is upstream text, not documentation for
> this repository — commands below referencing `submissions/examples/` belong to
> the original competition repo. For running **this** placer, see
...
*(The original challenge README animated simulated-annealing and
force-directed placement on ibm01. Those ~10 MB GIFs are not carried here — see
the [upstream repo](https://github.com/partcleda/partcl-macro-place-challenge)
for them.)*
...
The greedy placer achieves zero overlaps but makes no attempt to optimize wirelength or connectivity — your job is to do better! See [`SETUP.md`](SETUP.md) for the full API reference and `submissions/examples/` for working examples.
