...
# If you cloned without --recurse-submodules, initialize the TILOS
# MacroPlacement submodule now (required for evaluation).
git submodule update --init external/MacroPlacement

# Create the virtual environment and install the package (editable).
# Use `uv sync --extra abuplace` for the GPU placer's pinned torch.
uv sync

# One-time: fetch the prebuilt Xplace CUDA extensions (checksum-verified).
# Only needed to RUN the placer; the harness and tests work without it.
scripts/fetch_xplace_binaries.sh
```

> Disk note. `external/MacroPlacement` is a large checkout (~4 GB): it
> carries the evaluator, the ICCAD04 testcases, and the NG45/ASAP7 flow
> collateral. If you only need Tier-1 proxy evaluation, a sparse checkout of
> `CodeElements/Plc_client` and `Testcases/ICCAD04` is enough:
>
> ```bash
> git -C external/MacroPlacement sparse-checkout init --cone
> git -C external/MacroPlacement sparse-checkout set \
>     CodeElements/Plc_client Testcases/ICCAD04
> ```
...
├── abuplace/                 # The AbuPlace macro placer
├── external/
...
| Key | Description |
|-----|-------------|
| `proxy_cost` | Weighted sum: 1.0 x WL + 0.5 x density + 0.5 x congestion |
...
See [`abuplace/placer.py`](abuplace/placer.py) for this repository's placer, and
[`abuplace/TECH_DEEPDIVE.md`](abuplace/TECH_DEEPDIVE.md) for how it works.
...
uv run evaluate abuplace/placer.py --all
```
